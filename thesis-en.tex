...
cargo-semver-checks has been ran).

From those non-yanked, non-major releases which we were able to compile,
\textbf{in 470 of them we found one or more semver breaks}.
This means that on average every 1 in 30 releases that we checked had at least one semver break.
All in all, we found semver violations in 173 crates of the 1000 crates we scanned, slightly more than 1 in 6.
In total, different lints were triggered 655 times and reported 3305 items.
(one release can trigger multiple lints, each of them reporting multiple items in the source code that broke semver).

We verified each reported semver violation by constructing a \texttt{witness} -- a short Rust code
proving that a particular release indeed breaks semver.
Each witness can be compiled using one release of a crate,
but results in a compilation error for the other release.
...
result is a semver violation that is a true-positive.
...
\begin{table}[h]
	\centering
	\begin{tabular}{|
		>{\centering\arraybackslash}m{.55\linewidth} |
		>{\centering\arraybackslash}m{.15\linewidth} |
		>{\centering\arraybackslash}m{.15\linewidth} |
		>{\centering\arraybackslash}m{.15\linewidth} |
		}
		\hline
			{\textbf{name of the lint}} &
			{\textbf{number of found items}} &
			{\textbf{number of different triggered releases}} &
			{\textbf{number of different triggered crates}} \\
		\hline
		inherent\_method\_missing & $791$ & $41$ & $27$ \\
		enum\_variant\_added & $387$ & $140$ & $60$ \\
		constructible\_struct\_adds\_field & $343$ & $123$ & $34$ \\
		auto\_trait\_impl\_removed & $318$ & $57$ & $45$ \\
		struct\_missing & $291$ & $66$ & $40$ \\
		function\_missing & $265$ & $50$ & $33$ \\
		inherent\_method\_must\_use\_added & $239$ & $4$ & $4$ \\
		inherent\_method\_const\_removed & $139$ & $5$ & $3$ \\
		derive\_trait\_impl\_removed & $115$ & $11$ & $11$ \\
		enum\_variant\_missing & $112$ & $27$ & $18$ \\
		struct\_pub\_field\_missing & $79$ & $32$ & $16$ \\
		enum\_missing & $78$ & $26$ & $20$ \\
		trait\_missing & $45$ & $24$ & $19$ \\
		method\_parameter\_count\_changed & $22$ & $14$ & $12$ \\
		enum\_marked\_non\_exhaustive & $17$ & $4$ & $4$ \\
		struct\_repr\_c\_removed & $12$ & $3$ & $3$ \\
		constructible\_struct\_adds\_private\_field & $9$ & $7$ & $6$ \\
		inherent\_method\_unsafe\_added & $9$ & $3$ & $3$ \\
		function\_parameter\_count\_changed & $8$ & $4$ & $4$ \\
		function\_unsafe\_added & $8$ & $2$ & $2$ \\
		unit\_struct\_changed\_kind & $5$ & $3$ & $2$ \\
		enum\_tuple\_variant\_field\_missing & $4$ & $2$ & $2$ \\
		tuple\_struct\_to\_plain\_struct & $4$ & $2$ & $2$ \\
		enum\_tuple\_variant\_field\_added & $3$ & $3$ & $3$ \\
		enum\_repr\_int\_removed & $1$ & $1$ & $1$ \\
		enum\_struct\_variant\_field\_added & $1$ & $1$ & $1$ \\
		\hline
	\end{tabular}
\end{table}
...
from the registry of a particular crate and release,
we thought of running the tool on a large number of existing crates
to find semver violations and bugs (which mostly are false-positives or crashes).
...
(they have the same crate name as a dependency, but two different releases of it).
...
to the given variant. This source code compiles in the baseline, but does not compile
in the current (because the Rust compiler cannot find the variant), thus it is a valid witness.
...
\begin{itemize}
	\item the baseline compiles and the current does not compile, which means the witness has confirmed that there is a semver violation (a true-positive),
	\item both the baseline and current compile, which indicates a false-positive or a bug in one of the dependencies used by cargo-semver-checks,
