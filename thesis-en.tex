...
how we managed to obtain them, see section \ref{r:section_generating_witnesses}.

We considered only non-yanked releases of those crates which were published
after 2017-01-01 and are not a major change. From those releases,
...
Those releases are in 173 different crates (out of 1000).
...
Moreover, for each result we have viewed its source code to confirm
...
All of the detected true-positives (with their witnesses) can be found in a GitHub repository \cite{github-csv-results}.
They are provided in both human-readable and machine-readable form.
The remaining results (false-positives and bugs) are provided only in human-readable form.
...
\begin{itemize}
	\item the baseline compiles and the current does not compile, which means the witness has confirmed that there is a true-positive,
