...
\begin{itemize}
	\item Tomasz:
		\begin{itemize}
			\item Reported a bug with a wrong path to the generated rustdoc when passing
				a manifest path in the CLI \cite{issue_wrong_path_manifest_path}.
			\item Changed how the manifest of the baseline was being built and added
				selecting all features to baseline's manifest \cite{pr_changed_how_manifest_baseline_built},
				which resolved an issue found by a user of the tool
			...
			\item Renamed directories with queries and tests, thanks to which the directory
				structure of the project is now clear and more intuitive
			...
			\item Improved the tests -- split each test into separate pairs of crates
				(that are baseline and current crates for the tool),
				made it so that the tool runs the lint on all crates
				(which has detected multiple new false-positives),
				massively improved the readability and ease of adding new tests and lints,
				and improved the error messages when the test output didn't match,
				which speeds up the process of adding new lints \cite{pr_split_each_test}.
			\item Made a change in the tests that runs the tool on crates that didn't change
			...
			\item Added a new type of integration tests, which locally checks
				...
				(where the lint files weren't present in the binary)
			...
			\item Diagnosed a problem where the tests stopped working \cite{issue_tests_fail_unknown},
				...
				affected with those changes \cite{issue_strengthen_back}.
			...
			\item Changed the way in which the rustdoc of a crate was generated
			...
			\item Feported a false-positive which was related to re-exporting types, values and macros
	...
	\item Tomasz and Michał:
		\begin{itemize}
			\item Added a new lint (trustfall query, test crates, tests) to
				...
				and added a field in the in the adapter for the lint \cite{pr_added_field_structvariant}.
	...
	\item Michał:
		\begin{itemize}
			...
			\item Made a new release workflow in the CI that triggers when publishing
				a new version of the tool and uploads the new binary to GitHub releases,
...
% All of Mieszko's and Bartosz's PRs and Issues (open and closed)
% https://github.com/obi1kenobi/cargo-semver-checks/pull/155
% https://github.com/obi1kenobi/cargo-semver-checks/pull/158
% https://github.com/obi1kenobi/trustfall-rustdoc-adapter/pull/5
% https://github.com/obi1kenobi/cargo-semver-checks-action/pull/19
% https://github.com/obi1kenobi/cargo-semver-checks/pull/255
...
% https://github.com/obi1kenobi/cargo-semver-checks-action/pull/20
...
% https://github.com/obi1kenobi/cargo-semver-checks/pull/389
% https://github.com/obi1kenobi/cargo-semver-checks/issues/276
% https://github.com/obi1kenobi/cargo-semver-checks/pull/407
% https://github.com/obi1kenobi/cargo-semver-checks-action/pull/21
% https://github.com/obi1kenobi/cargo-semver-checks-action/pull/25
