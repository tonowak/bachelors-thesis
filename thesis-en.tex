...
but the detection mechanism and existing lints are written
...
\begin{itemize}
	\item it is reasonable to use the tool in continuous integration
		-- the tool reports semver breaks only when
		its output shouldn't be ignored and requires manual inspection,
	\item we're avoiding a scenario in which
		when an user encounters false-positives, they become frustated
		and their trust in the tool is reduced, so they are
		less likely to use the tool again.
\end{itemize}

Similarly to the Rust compiler, the reports contain
detailed information about the found issues, including
the line of code in which the issue was found, a short and long description
of the triggered lint and possibly reference links to the Rust documentation
...
That would mean that the tool would be available to all Rust programmers
through a cargo subcommand and would be a huge step towards reducing the number of
package releases with a semver issue.
...
or sometimes even review or develop the tool.
...
via GitHub Actions to automatically check whether
...
current crate should be compared, or
...
that checks whether a given git branch haven't violated semver with recent changes.
...
\begin{itemize}
	...
	\item the codebase was not in a state where new contributors could easily implement their first changes
		...
		and the code lacked comments that explained some of the logic and decisions behind the code.
\end{itemize}
