...
\end{figure}
The release of binaries was a big change for how the tool is used and had its effect on
the project's downloads, too. Originally a repository using cargo-semver-checks would most often
...
recent one. However, with pre-builds now available, most projects download the newest version
of the tool only once, add the binaries to their repository and use them instead. This has lowered
the average downloads of cargo-semver-checks, but by no means decreased the tool's popularity --
quite the contrary, as the current number of over 2 000 downloads per release better reflects
how many repositories actively use cargo-semver-checks.

Another measure of cargo-semver-checks' recognition among the community are GitHub stars.
Every user can star any number of repositories they like, whereas they want to show
...
is impossible to point out a single, main factor, there are several (with no specific order) that
...
\begin{itemize}
	...
	\item introduction of the pre-built executable binaries with their convenience of use,
...
cargo-semver-checks into other systems -- the tool is available in nixOS, has been introduced
into release managers like release-plz, and is used by Arch Linux distribution.
With those and many more usage scenarios, cargo-semver-checks has proven its versatility
in the vast, diverse world of semantic versioning.
