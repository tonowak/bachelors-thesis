...
When our work on the project started, the tool had already gained these features, which made the
action obsolete. It was still handy for being used in continuous integration, as it
...
and possible to be run only on Linux hosts.

The need for a redesign was clear, so we decided to use the fact that a GitHub action might
...
\begin{itemize}
	...
	\item Using Node.js makes it possible to depend on variety of npm packages, in particular the GitHub Actions
...
\end{itemize}
The new version of the action not only fixes the problems of the previous one, but also
...
\begin{itemize}
	...
	\item input \texttt{manifest-path}, which allows to specify the path to the \texttt{Cargo.toml} file
		...
		the action is run,
	\item input \texttt{verbose}, which might be used to enable the verbose output of the tool,
...
\begin{itemize}
	\item The cargo-semver-checks is downloaded as a precompiled binary instead of being built from source.
	\item The rustdoc of the baseline version is cached between the action runs, so that it is generated
	...
	\item When possible, downloads from crates.io are being made using recently developed "sparse" protocol
		\cite{crates_io_sparse_protocol}.
\end{itemize}

Along with the new version of the action we also developed an extended suite of action tests
(not to be confused with the test suite of cargo-semver-checks). The current CI workflow
of the projects consists of over 30 checks, which are:
\begin{itemize}
	...
	\item veryfying whether the action works on Linux, Windows and macOS with different versions of
		Rust installed,
	\item testing each of the action inputs by running it on a test repository,
...
but also in the cargo-semver-checks itself, and will make it easier to maintain the project in the future.
