...
cargo-semver-checks goal is to detect semver violations within Rust crates. Given two versions
of the same crate -- baseline (considered valid) and current (new) -- the tool parses them using
Trustfall to introduce a layer of abstraction that remains constant (functions, structs, enums,
variants etc.). After that, the versions are compared using all of the implemented lints to find
the breaking public API changes. They are the core component of the project and are carefully designed
so that one break can only be reported by one, precise lint.

% Possibly introduce a pagebreak here before the final merge into main, depending on how the layout
% looks like -- it would be great to have the header, image and bullet list all on one page!
...
\begin{figure}[h]
	...
	\caption{Example of a lint's query}
\end{figure}
...
\begin{itemize}
	...
	\item \texttt{@optional} -- hides all of the underlying \texttt{@output}, requiring additional
		execution flags to show it,
	\item \texttt{... on Enum} -- \texttt{...} inlines the query to one, specified kind of items,
		in this case Enums,
