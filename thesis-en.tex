...
\begin{itemize}
	...
	\item the manifests of the baseline and current crates can be different,
		...
		of the project),
	\item the tests became more organised, uncovering some bugs
		-- previously each test sourcecode had a lot of branch conditions that
		made it difficult to find mistakes
		(e.g. when the code introduced an item in \say{current}
		that was then removed in \say{baseline}).
\end{itemize}
What is more, because those tests are also good for testing the JSON rustdocs
(they check a variety of rustdocs and lots of different kinds of items),
we have developed more extensive ones than the
Rust team responsible for developing rustdoc.
Thus they decided to adapt ours in their testsuite
(by running the tool's tests on their newest versions of rustdocs) \cite{issue-rustdoc-our-tests}.

We also wrote a new kind of tests to ensure that some bugs found by us or reported by the users have been fixed.
...
with various commandline arguments and they detected that it wrongly handled some arguments.

While those integration tests were more appropriate to be written as a GitHub workflow job
(simply because cloning the user's git repo takes time and memory, it requires internet connection
...
For example, there was a bug where the crate pair tests did not include the lints in the produced binary (which fortunately was noticed before going to production).
