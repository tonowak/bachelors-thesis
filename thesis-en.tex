...
works with all recent versions of Rust (and consequently with all recent versions of
rustdoc). Therefore, the trustfall-rustdoc-adapter is developed on several different
branches, each of them handling a~specific rustdoc version.

trustfall-rustdoc brings them together in form of a single tool. Given the path to the rustdoc JSON
output, trustfall-rustdoc loads the file, detects which rustdoc version was used and chooses the
corresponding trustfall-rustdoc-adapter branch to handle the Trustfall queries.
