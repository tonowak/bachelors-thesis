...
variants etc.). After that the versions are compared using all of the available lints to find
possible public API changes. They are the core component of the project and are carefully designed
so that one break can only be reported by one, specialized lint.
...
Each query defines the rules for filtering the baseline and current versions of a crate. Consider
...
The query above is part of the lint seen in figure \ref{fig:enum_missing_lint} and can
be interpreted as:
