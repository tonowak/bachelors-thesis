...
Since we are aware of how important it is to maintain high quality of the project's codebase,
our contributions to the tool were not just limited to developing new code. We have introduced
several changes and refactors that not only made the codebase easier to navigate and expand,
but also prepared the tool to be even more successful in the future.
...
\begin{itemize}
	...
	of code in several different files. What is more, the tests for the new lint had to be written
	in a single file where all the baseline and current code fragments had to be preceded by
	a macro differentiating them. Although there were instructions available, they lacked
	...
	and implement it together with suitable tests -- all the minor file changes are taken care of
	...
	as if they were developing a crate (the difference being instead of avoiding semver violations
...
Shortly after this refactor we changed how the tests are run. Instead of limiting one lint to just
its dedicated test cases we decided to check it against all available tests from other
lints as well. This allowed us to detect and eliminate some unnoticed false-positives,
but even more importantly lowered the risk of new ones occuring in the tool's further development.
...
With time we kept on introducing even more changes, such as:
\begin{itemize}
	\item excluding yanked releases from the available versions when manually selecting the
	baseline for running the tool. This protects the users from accidentally comparing their
	current version of the project against ones that are not publicly accessible anymore,
	what could lead to breaking the semver,
	\item refactoring the \texttt{baseline.rs} file to make future additions easier. With this
...
% 	\#203 -- pairs of crates instead of single, complicated crates
% 		(writing tests is 10x easier thanks to this)
% 	\#204 -- only one lint list
% 	\#240 -- improved CONTRIBUTING.MD
% 	\#239 -- script that creates everything for the new contributor

% 	\item describe other overall changes that prepares the tool to be more successful in the future
% 	\#206 -- run each lint on all test crates rather than just its own tests
% 		(less false-positives etc.)
% 	\#208 -- binaries to avoid downloading the repository all the time in actions
% 	\#255 -- exclude yanked releases when choosing baseline version

% 	\item describe refactors and other changes that made the codebase better
% 		\#340 -- refactor of baseline.rs for easier changes in the future
% 		\#341 -- use placeholders for rustdoc generation
% \end{itemize}
