...
We have discovered that more than 1 in 6 of the largest Rust libraries have broken a multitude
of projects by publishing new releases, causing their users' code to suddenly stop compiling.
In such cases, the packages' maintainers have to delete the faulty releases or create patches
to fix them and the users have to temporarily downgrade the library's version. This is a serious
concern as the problem is alarmingly common but has never been satisfactorily resolved so far.

The following document describes the development of an open-source tool called
\texttt{cargo-semver-checks} that solves this problem by detecting changes that are
not backwards-compatible in Rust libraries' codes before releasing a new version.
...
this approach, the project is much more sustainable than other attempts of this kind among
other languages, which are not maintained anymore.
...
made it difficult for the community to use the tool and we further improved the codebase to attract
new contributors.

Furthermore we present the results of running cargo-semver-checks on a large number
of popular crates. This research proved that the problem is far more common than expected.
In addition to gathering statistics on occurrences of different semantic versioning breaks,
it revealed the most frequent ones which the developers should pay closer attention to
in their projects.
...
use cargo-semver-checks on a daily basis to prevent semantic versioning breaks in their
...
\begin{itemize}
    \item {\ttfamily enum\_marked\_non\_exhaustive}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/155
    \item {\ttfamily enum\_struct\_variant\_field\_missing}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/156
    \item {\ttfamily function\_parameter\_count\_changed}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/170
    \item {\ttfamily tuple\_struct\_to\_plain\_struct}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/249
    \item {\ttfamily trait\_unsafe\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/252
    ...
    \item {\ttfamily enum\_must\_use\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/278
    \item {\ttfamily struct\_must\_use\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/279
    \item {\ttfamily trait\_must\_use\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/280
    \item {\ttfamily function\_must\_use\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/281
    \item {\ttfamily inherent\_method\_must\_use\_added}
        ...
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/283
\end{itemize}
...
% https://github.com/obi1kenobi/cargo-semver-checks/pull/171
...
\say{current} and \say{baseline} codes),
...
in the \say{current} (because the Rust compiler cannot find the variant), thus it is a~valid witness.
...
\begin{itemize}
	\item the \say{baseline} compiles and the \say{current} does not compile, which means the witness has confirmed that there is a~semver violation (a~true-positive),
	\item both the \say{baseline} and \say{current} compile, which indicates a~false-positive or a~bug in one of the dependencies used by cargo-semver-checks,
	\item the \say{baseline} does not compile, which means that the witness was constructed incorrectly.
\end{itemize}
...
\begin{center}
	\begin{longtable}{| p{6.35cm} | p{8.2cm} |}
		...
		\hline
			...
			\end{tabular} &
			\multicolumn{1}{Sl|}{\parbox{8.2cm}{
				a~missing function/method call with an~adequate number of \texttt{todo!()} arguments.
				This technique creates valid witnesses, even though it generates warnings that
		...
		\begin{itemize}
			...
			\item Developed \texttt{semver-crater} that runs cargo-semver-checks on the most popular crates
				...
				\begin{itemize}
					...
					\item filtering releases which were affected by the \say{doc-hidden} false-positives,
...
% However, cargo-semver-checks is not only used by the large, popular and complex projects.
% The entire Rust community has shown great interest in using the tool as well -- as of May 2023,
% cargo-semver-checks has over 63 000 downloads from the crates.io registry \cite{downloads-total}.
% This number can be split down into three components:
% \begin{itemize}
% 	\item over 22 900 downloads before we started contributing to the project (period of 5 months),
% 	\item over 32 700 downloads during our contributions, before the release of pre-built executable
% 		binaries (period of 3 months),
% 	\item over 7400 downloads during our contributions, after the release of pre-built executable
% 		binaries (period of 3 months).
% \end{itemize}
% \begin{figure}[h]
% 	\centering
% 	\includegraphics[width=\linewidth]{crates-io-downloads-total.png}
% 	\caption{cargo-semver-checks on crates.io, May 2023}
% \end{figure}
% The release of pre-built binaries was a~big change for how the tool is accessed and had its effect on
% the project's downloads. Originally a~repository using cargo-semver-checks would most often
% download the same version multiple times. For example, every time a~CI run took place the tool
% would be downloaded from crates.io and built locally from scratch. This resulted in a~big variation
% of downloads per release, depending on factors such as how long it has been since the most
% recent one. However, with pre-built binaries now available, more and more projects
% decide to get them directly from the GitHub repository for CI usage
% rather than download the tool from crates.io.
% This has lowered
% the average downloads of cargo-semver-checks, but by no means decreased the tool's popularity.

% Another measure of cargo-semver-checks' recognition among the community are GitHub stars.
% Every user can star any repositories they like, no matter whether they want to show
% their appreciation of one's project or just bookmark it for easier finding in the future.
% When we started working on the tool in October 2022, it had just over 200 stars.
% 7 months have passed and after our contributions the star count has almost doubled,
% reaching above 400.
% \begin{figure}[h]
% 	\centering
% 	\includegraphics[width=0.75\linewidth]{github-star-history-graph.png}
% 	\caption{cargo-semver-checks star history as of May 2023}
% \end{figure}
...
% \bibitem{downloads-total} crates.io,
% 	\textit{cargo-semver-checks on crates.io} (2023) \\
