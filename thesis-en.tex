...
The continuosly expanding list of lints is the heart of the project.
...
\begin{itemize}
    \item {\ttfamily enum\_marked\_non\_exhaustive}
        -- detects marking {\ttfamily Enum} as {\ttfamily \#[non\_exhaustive]},
    ...
    \item {\ttfamily function\_parameter\_count\_changed}
        -- detects functions with changing parameter count,
    ...
    \item {\ttfamily trait\_unsafe\_added}
        -- detects marking {\ttfamily Trait} as {\ttfamily unsafe},
    ...
    \item {\ttfamily enum\_must\_use\_added}
        -- detects marking {\ttfamily Enum} as {\ttfamily \#[must\_use]},
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/278
    \item {\ttfamily struct\_must\_use\_added}
        -- detects marking struct as {\ttfamily \#[must\_use]},
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/279
    \item {\ttfamily trait\_must\_use\_added}
        -- detects marking {\ttfamily Trait} as {\ttfamily \#[must\_use]},
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/280
    \item {\ttfamily function\_must\_use\_added}
        -- detects marking function as {\ttfamily \#[must\_use]},
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/281
    \item {\ttfamily inherent\_method\_must\_use\_added}
        -- detects marking method as {\ttfamily \#[must\_use]},
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/283
\end{itemize}
To enable the new lints, we had to do some work in the Trustfall rustdoc adapter. 
It mainly resolved around extending the schema. Most of the time, the changes were simple. 
The challenge there, was to design it in a way, that will not become a roadblock in the future.

One notable exception, is that the schema for attributes only exported its value as a string, 
so traversing its inner tags in lints was not possible. 
Because of this, we had to introduce a breaking change there,
making it more future-proof this time, and adjust the existing lints.
