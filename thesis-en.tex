...
Cargo-semver-checks is a CLI tool for detecting semantic versioning violations within Rust crates.
To do so, a new version of a crate undergoes multiple lint checks, which are meant to find
public API changes between the base and new version of the crate. All of the data needed to compare
the two versions is retrieved from rustdoc using Trustfall, which allows to introduce a layer
of abstraction that remains constant (functions, structs, enums, variants etc.). To learn about the
tool's practical usage and long-term goals, see chapter \ref{r:chapter_vision}.
