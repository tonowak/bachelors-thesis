...
\begin{itemize}
    ...
    \item {\ttfamily function\_parameter\_count\_changed}
        ...
        changing the way the function umst be called.
    ...
    \item {\ttfamily tuple\_struct\_to\_plain\_struct}
        ...
        changing the way the struct must be initailized.
    ...
    \item {\ttfamily trait\_unsafe\_removed}
        ...
        removing the ability to do a unsafe {\ttfamily impl}.
        % https://github.com/obi1kenobi/cargo-semver-checks/pull/252
    \item {\ttfamily enum\_must\_use\_added}
        -- detects marking an {\ttfamily Enum} with the {\ttfamily \#[must\_use]} attribute,
        removing the ability to ignore it, when returned from a function.
    ...
    \item {\ttfamily trait\_must\_use\_added}
        ...
        removing the ability to ignore an object with dynamic type implementing
        this trait, when returned from a function.
...
It mainly resolved around extending its schema and the changes were simple most of the time.
The real challenge there was to design it in a way that would not become troublesome to work with in the future.
...
and do minor adjustments the existing lints, 
making it more future-proof this time.
