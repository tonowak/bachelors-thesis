...
features that a crate should be built with for the checks. This is important
because some functionalities may change between features. What is more,
crates often have a feature that breaks semver by design, what
we discovered when working on the semver-crater \ref{r:section_results_semver_crater}.
Now the users can adjust them for their specific use cases.

The new default behaviour is to include all the features,
filtering out heuristicly the ones that break semver by looking at 
the feature's name. For example, feature {\ttfamily unstable} would not be
...
thus we had to make sure that the cache is not used if the feature
configs differ.
