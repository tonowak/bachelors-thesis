...
\begin{center}
	\begin{tabular}{p{4cm} p{11cm}}
		\textbf{lint type} & \textbf{witness description} \\ \hline
		struct or enum item removed & a (possibly generic) function that takes the item as an argument \\ \hline
		trait removed & a generic function (with possibly multiple generic arguments) that takes
			a generic argument satisfying the trait \\ \hline
		function or method changed (parameter count changed or removed) & executing the function with
			...
			but it's not needed to show a semver break) \\ \hline
		trait implementation removed & a helper function that takes a generic argument satisfying
			the given trait an a (possibly generic) function that takes the item and passes it to the helper function \\ \hline
		enum variant missing & deconstructing the enum with an \texttt{if} to the given variant \\ \hline
		enum variant added, enum marked non-exhaustive or struct field added & pattern-matching
