...
In order to check two versions of a crate for semver violations, our tool needs a consistent method
for generating the data to be compared. This is achieved by using rustdoc (from the official Rust toolchain) for
generating the documentation for Rust projects. However, the documentation itself does not come in
a form that is easy to analyze or compare. For this purpose, our tool uses Trustfall to parse the
output received from rustdoc into a more managable form.

Trustfall is a special query engine, designed to query any data source or a combination of data
sources such as APIs, raw files (JSON, CSV, etc.), git commits and many more. Its main idea is to
represent the data as an organised graph with vertices and edges that the engine can traverse. To
navigate during the search, it follows consecutive filtering rules defined by the user in a special
\texttt{.ron} (rusty object notation) file. These files are formally named \texttt{lints}.

\begin{figure}[h]
	\centering
	\caption{Example of a lint}
	\includegraphics[width=\linewidth]{lint-example.png}
\end{figure}

In cargo-semver-checks, each lint consists of the following fields:
\begin{itemize}
	\item \texttt{id} -- a short, unique name identifying the lint, most often matching the
		filename that contains the lint,
	\item \texttt{human\_readable\_name} -- a longer, more informative name,
	...
	\item \texttt{reference\_link} -- a link to the official Rust documentation coresponding to
		the issue (if available),
	\item \texttt{query} -- the filtering rules responsible for comparing two versions and
		detecting semver violations (more detail in section \ref{r:section_new_lints}),
	\item \texttt{arguments} -- optional, user-defined constants for the query,
	\item \texttt{error\_message} -- the exact problem and some of its possible causes,
	\item \texttt{per\_result\_error\_template} -- the exact location of the code that broke semver.
\end{itemize} \bigskip
...
\begin{itemize}
	\item \textbf{cargo-semver-checks} -- a CLI tool that runs lints (Trustfall queries),
	\item \textbf{trustfall-rustdoc-adapter} -- implements the Trustfall interface for one specific
		rustdoc version,
	\item \textbf{trustfall-rustdoc} -- allows to run Trustfall queries over rustdoc regardless of
		the version,
	\item \textbf{cargo-semver-checks-action} -- the tool in form of a GitHub Action.
\end{itemize}

\begin{figure}[h]
	\centering
	\caption{Diagram of the project structure (TODO: make it more pretty, add github action)}
